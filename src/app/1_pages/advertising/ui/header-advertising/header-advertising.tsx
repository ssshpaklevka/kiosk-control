...
import { useCreateHeaderBanner } from "../../hooks/use-header-banner";
import { useStores } from "../../hooks/use-stores";
import { CreateBanner } from "../../types/adevrtising";
import { TableHeaderAdvertising } from "./table-header-advertising";
...
export const HeaderAdvertising = () => {
  ...
  const createHeaderBanner = useCreateHeaderBanner();
  ...
  const validateFile = async (
    ...
    if (file.type === "image/webp") {
      return new Promise((resolve) => {
        const img = new Image();
        img.onload = () => {
          // Проверка размеров для баннера в шапке заказа
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error("Ошибка создания рекламы в шапке:", error);
    } finally {
    ...
    <>
      <div className="flex flex-row gap-8">
        <div className="flex flex-col gap-2">
          <p className="text-xl">Баннер в шапке заказа</p>
          ...
          >
            <div className="w-full flex rounded-3xl justify-center">
              <div
                ...
                className="w-[338px] h-[122px] mt-[14px] bg-red-600 rounded-3xl bg-cover bg-center bg-no-repeat"
              ></div>
            </div>
        ...
        <div className="flex flex-col gap-4 min-w-80">
          <div>
            <p className="text-lg font-medium mb-2">
              Изменить баннер в шапке заказа
            </p>
